...
from regix.benchmark import BackendProbe, BenchmarkReporter, BenchmarkResult, BenchmarkSuite, CLIProbe, ImportProbe, ThroughputProbe, UnitTestProbe, _fmt_time, _make_config_parse_probe, benchmark_library, build_regix_suite, main
...
class TestBenchmarkSuite:
    ...
    def test_run_probes(self):
        ...
        mock_probe.run.return_value = BenchmarkResult(name='mock', suite='test', elapsed=0.1)
...
class TestThroughputProbe:
    ...
    def test_with_setup_teardown(self):
        ...
        probe = ThroughputProbe(label='lifecycle', fn=lambda: None, n=1, setup=lambda: state.update(setup=True), teardown=lambda: state.update(teardown=True))
        probe.run()
...
class TestCLIProbe:
    ...
    def test_echo(self):
        import sys
        probe = CLIProbe([sys.executable, '-c', "print('hello')"], label='echo test', repeat=1, threshold=10.0)
        result = probe.run()
        assert result.status == 'PASS'

    def test_command_not_found(self):
        probe = CLIProbe(['__nonexistent_cmd_xyz__'], label='missing cmd', repeat=1)
        result = probe.run()
        assert result.status == 'ERROR'
...
class TestBenchmarkLibrary:
    ...
    def test_with_cli_commands(self):
        import sys
        suite = benchmark_library('os', cli_commands=[[sys.executable, '-c', 'print(1)']], threshold_cli=10.0)
...
class TestMain:
    ...
    def test_main_json(self, mock_build, capsys):
        ...
        mock_suite.run.return_value = [BenchmarkResult(name='t', suite='s', elapsed=0.1)]
    ...
    def test_main_plain(self, mock_build, capsys):
        mock_suite = MagicMock()
        mock_suite.run.return_value = [BenchmarkResult(name='ok', suite='s', elapsed=0.5, threshold=1.0)]
        mock_build.return_value = mock_suite
    ...
    def test_main_with_threshold_override(self, mock_build, capsys):
        mock_suite = MagicMock()
        mock_suite.run.return_value = [BenchmarkResult(name='slow', suite='s', elapsed=10.0, unit='s')]
        mock_build.return_value = mock_suite
