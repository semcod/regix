...
# Bootstrap
# ---------------------------------------------------------------------------
_ROOT = Path(__file__).parent.parent  # regix/ project root
...
class ImportProbe(BenchmarkProbe):
    ...
    def run(self) -> BenchmarkResult:
        ...
        best = min(times)
        return BenchmarkResult(
            name=self.label, suite=self.suite, elapsed=best,
            threshold=self.threshold,
...
class CLIProbe(BenchmarkProbe):
    ...
    def run(self) -> BenchmarkResult:
        ...
        for _ in range(self.repeat):
            ...
            try:
                proc = subprocess.run(
                    self.command, capture_output=True, cwd=self.cwd,
                    timeout=120, env=env,
                )
                elapsed = time.perf_counter() - t0
...
class UnitTestProbe(BenchmarkProbe):
    ...
    def run(self) -> BenchmarkResult:
        cmd = [
            ...
            str(self.test_path), "--tb=no", "--no-header", "-q",
            "--no-cov",
            *self.pytest_args,
        ]
        ...
        return BenchmarkResult(
            name=self.label, suite=self.suite, elapsed=elapsed,
            threshold=self.threshold, extra=extra, error=error,
        )
...
class ThroughputProbe(BenchmarkProbe):
    ...
    def __init__(
        ...
        threshold_ops: Optional[float] = None,
...
class BackendProbe(BenchmarkProbe):
    ...
    def run(self) -> BenchmarkResult:
        ...
        try:
            import regix.backends as _bk_mod  # noqa: F401
            from regix.backends import get_backend
            from regix.config import RegressionConfig
        except ImportError as e:
            return BenchmarkResult(
                name=self.label, suite=self.suite,
                elapsed=0.0, error=f"import error: {e}",
                threshold=self.threshold,
            )
        ...
        if backend is None:
            return BenchmarkResult(
                name=self.label, suite=self.suite,
                elapsed=0.0, error=f"backend '{self.backend_name}' not registered",
                threshold=self.threshold,
            )
        if not backend.is_available():
            return BenchmarkResult(
                name=self.label, suite=self.suite,
                elapsed=0.0, error=f"backend '{self.backend_name}' not available",
                threshold=self.threshold,
            )
        ...
        try:
            ...
            # Read all generated files into RAM for in-memory analysis
...
# ---------------------------------------------------------------------------
# Regix-specific benchmark suite
# ---------------------------------------------------------------------------
...
def build_regix_suite() -> BenchmarkSuite:
    ...
    suite.add(CLIProbe(
        [sys.executable, "-m", "regix", "compare", "HEAD~1", "HEAD", "--format", "json"],
...
def _make_config_parse_probe() -> ThroughputProbe:
    ...
    return ThroughputProbe(
        ...
        setup=setup,
        teardown=teardown,
        threshold_ops=50.0,  # at least 50 parses/s
    )
...
def _make_snapshot_probe() -> ThroughputProbe:
    ...
    return ThroughputProbe(
        ...
        n=3,
        setup=setup,
        threshold_s=30.0,  # total 3 captures in ≤30s
    )
...
def _make_compare_probe() -> ThroughputProbe:
    ...
    return ThroughputProbe(
        ...
        n=10,
        setup=setup,
        threshold_ops=5.0,  # at least 5 compares/s
    )
...
def _make_gates_probe() -> ThroughputProbe:
    ...
    return ThroughputProbe(
        ...
        n=50,
        setup=setup,
        threshold_ops=20.0,  # at least 20 gate checks/s
    )


# ---------------------------------------------------------------------------
# Reusable helper for benchmarking any library
# ---------------------------------------------------------------------------
