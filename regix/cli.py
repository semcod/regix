...
def compare(
    ...
    local: bool = typer.Option(
        False, "--local", help="Compare REF_A against the current working tree"
    ),
    config: Optional[str] = typer.Option(None, "--config", help="Path to regix.yaml"),
    fmt: str = typer.Option(
        "rich", "--format", "-f", help="Output format: rich | json | yaml | toon"
    ),
    output: Optional[str] = typer.Option(
        None, "--output", "-o", help="Write report to file/dir"
    ),
    metric: Optional[list[str]] = typer.Option(
        None, "--metric", "-m", help="Filter to specific metric(s)"
    ),
...
def history(
    ...
    metric: Optional[list[str]] = typer.Option(
        None, "--metric", "-m", help="Metrics to include"
    ),
    fmt: str = typer.Option(
        "rich", "--format", "-f", help="Output format: rich | json | yaml | csv"
    ),
...
def diff(
    ...
    metric: Optional[list[str]] = typer.Option(
        None, "--metric", "-m", help="Filter metric(s)"
    ),
    config: Optional[str] = typer.Option(None, "--config", help="Path to regix.yaml"),
    workdir: str = typer.Option(".", "--workdir", "-w", help="Project root"),
) -> None:
