...
class RegressionReport:
    ...
    def _toon_regression_section(label: str, items: list[Regression]) -> list[str]:
        ...
        for r in items:
            ...
            lines.append(
                f"  {r.file},{r.symbol or '(module)'},{r.metric},{r.before},{r.after},{sign}{r.delta}"
            )
        lines.append("")
        return lines
    ...
    def to_toon(self) -> str:
        ...
        lines.append(
            f"  compared: {total_symbols} symbols  errors: {self.errors}  warnings: {self.warnings}  improvements: {len(self.improvements)}"
        )
        lines.append("")
        ...
        if self.improvements:
            lines.append(
                f"IMPROVEMENTS[{len(self.improvements)}]{{file,symbol,metric,before,after,delta}}:"
            )
            for i in self.improvements:
                sign = "+" if i.delta > 0 else ""
                lines.append(
                    f"  {i.file},{i.symbol or '(module)'},{i.metric},{i.before},{i.after},{sign}{i.delta}"
                )
            lines.append("")
        return "\n".join(lines)
