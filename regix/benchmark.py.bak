...
class UnitTestProbe(BenchmarkProbe):
    ...
    def run(self) -> BenchmarkResult:
        ...
        try:
            proc = subprocess.run(
                cmd, capture_output=True, text=True, cwd=self.cwd, timeout=300
            )
            elapsed = time.perf_counter() - t0
        except subprocess.TimeoutExpired:
            return BenchmarkResult(
...
class ThroughputProbe(BenchmarkProbe):
    ...
    def run(self) -> BenchmarkResult:
        ...
        try:
            ...
            return BenchmarkResult(
                name=self.label, suite=self.suite, elapsed=0.0, error=str(e)
            )
        finally:
            if self.teardown:
                try:
...
def build_regix_suite() -> BenchmarkSuite:
    ...
    if tests_dir.exists():
        ...
        for test_file in sorted(tests_dir.glob("test_*.py")):
            suite.add(
                UnitTestProbe(
                    test_file, label=f"pytest {test_file.name}", threshold=30.0
                )
            )
    for bk_name, thresh in [
        ...
        suite.add(
            BackendProbe(
                backend_name=bk_name, file_count=20, file_size_kb=1.0, threshold=thresh
            )
        )
...
def _make_snapshot_probe() -> ThroughputProbe:
    ...
    return ThroughputProbe(
        label="snapshot.capture(HEAD)", fn=fn, n=3, setup=setup, threshold_s=30.0
    )
...
def _make_compare_probe() -> ThroughputProbe:
    ...
    return ThroughputProbe(
        label="compare(HEAD, HEAD)", fn=fn, n=10, setup=setup, threshold_ops=5.0
    )
...
def _make_gates_probe() -> ThroughputProbe:
    ...
    return ThroughputProbe(
        label="check_gates(HEAD)", fn=fn, n=50, setup=setup, threshold_ops=20.0
    )
