...
def _function_count_drop(
    mod_before: SymbolMetrics | None, mod_after: SymbolMetrics | None
...
def _largest_function(
    funcs: dict[str, SymbolMetrics],
) -> tuple[str, SymbolMetrics, int] | None:
...
def _check_god_function(
    ...
    if largest_len < config.god_func_length_min or largest_len <= avg_before * 1.5:
        return []
    severity = (
        "error" if largest_len > avg_before * 2.5 or fc_dropped >= 2 else "warning"
    )
    return [
        ArchSmell(
            smell="god_function",
            ...
            severity=severity,
            detail=f"Function count dropped {fc_before}→{fc_after} ({drop_ratio:.0%}); '{largest_sym}' is {largest_len} lines (avg was {avg_before:.0f})",
            ref_before=ref_b,
            ref_after=ref_a,
        )
    ]
...
def _check_stub_regression(
    ...
    return ArchSmell(
        ...
        severity=severity,
        detail=f"Function shrank {len_b}→{len_a} lines ({len_a / len_b:.0%}) with no external calls",
        ref_before=ref_b,
        ref_after=ref_a,
    )
...
def _check_no_delegation(
    ...
    return ArchSmell(
        ...
        line=m_after.line_start,
        severity="warning",
        detail=f"'{sym}' lost all delegation: 0 params, 0 calls (before: {', '.join(was_before)})",
        ref_before=ref_b,
        ref_after=ref_a,
    )
...
def _check_logic_density_drop(
    ...
    return ArchSmell(
        ...
        line=m_after.line_start,
        severity=severity,
        detail=f"Logic density dropped {ld_b:.2f}→{ld_a:.2f} (min: {config.min_logic_density})",
        ref_before=ref_b,
        ref_after=ref_a,
    )
...
def _check_cohesion_loss(
    ...
    return ArchSmell(
        ...
        line=m_after.line_start,
        severity="warning",
        detail=f"Node type diversity dropped {div_b}→{div_a} unique statement types (min: {config.min_node_diversity})",
        ref_before=ref_b,
        ref_after=ref_a,
    )
